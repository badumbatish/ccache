...
if ! $COMPILER_USES_MSVC; then
    ...
    $CCACHE_COMPILE -c -Wp,-U_FORTIFY_SOURCE,-D_FORTIFY_SOURCE=2 test.c
    expect_stat direct_cache_hit 0
    expect_stat preprocessed_cache_hit 0
    expect_stat cache_miss 1
...
exec $REAL_COMPILER "\$@"
EOF
        chmod +x wrapped-gcc.sh

        $CCACHE ./wrapped-gcc.sh -c test.c
        expect_stat direct_cache_hit 0
        expect_stat cache_miss 1
...
EOF
        ...
        # ccache reads the reported directory as a native path, so on Windows
        # it must be in native form like MinGW GCC prints it, not the MSYS
        # form of $PWD.
...
#!/bin/sh
case "\$*" in
    *-print-search-dirs*) echo "install: $native_pwd/installdir/x86_64/13/"; exit 0 ;;
esac
for specs in "$native_pwd/$versioned" "$native_pwd/$unversioned"; do
    if [ -f "\$specs" ]; then
        set -- -specs="\$specs" "\$@"
    fi
done
