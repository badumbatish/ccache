...
[#config_safe_direct_mode]
*safe_direct_mode* (*CCACHE_SAFEDIRECT* or *CCACHE_NOSAFEDIRECT*, see _<<Boolean values>>_ above)::

    If true, the direct mode also detects header files that appear in
    directories searched before the one where an included header was found
    (see _<<The direct mode>>_). Only GCC and Clang are supported. The default
    is false.
...
compilation. For GCC and Clang (but not for MSVC-compatible drivers like
clang-cl), ccache also asks the preprocessor for its header search directories
and records, for each include file, where a file with the same name would have
been found in the directories searched before the one the include file was found
in, including the directory of the including file. Header names probed with
`+__has_include+` are treated the same way. A result is only used if none of
those paths exist. Still, ccache can't check with 100% accuracy if the existence
of a new header file should invalidate a result. In practice, the direct mode is
safe to use in the absolute majority of cases.
...
* `+__has_include+` or `+__has_include_next+` with a macro operand is present in
  the source code
